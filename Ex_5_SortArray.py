...
class Array(object):
   ...
   def median(self): #self returns the contents of the array.  self.__nItems returns the number of items in the array
      total = len(self) #find the length of the array. 
      mid = total // 2 #finds the middle of the array. the // performs floor division meaning it roudns resurt to the nearest whole number
      if total % 2 == 0: #if total is even number then:
         return (self.get(mid - 1) + self.get(mid)) / 2 #(n+1)/2
         #self.get(mid - 1) and self.get(mid) are used to get the values of the middle elements. 
         #We used .get to access an element in the array by index.
         #For data encapsulation purposes, the Array class you're utilizing does not allow direct indexing of its internal array.
      else: #else, if total is odd then:
         return self.get(mid) #Since the total number is odd, return the middle value since odd numebers have a definite middle value
